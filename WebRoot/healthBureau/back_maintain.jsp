...
<!DOCTYPE html>    
    ...
    <body>
        <form action="<c:url value='/healthBureau/updateOrder_back.action' />"  method="post" id="myForm" >
       ...
       <%--  <center>
         ...
         <a href="#update" role="button" class="btn btn-primary" data-toggle="modal">修改</a>
          <div id="update" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
 			 ...
 			 <div class="modal-body">
   				 <p class="text-warning">是否修改该药品吗？</p>
 			 </div>
  			<div class="modal-footer">
   			 <button class="btn" data-dismiss="modal" aria-hidden="true">取消</button>
   			 <button class="btn btn-primary" onclick="document.getElementById('myForm').submit();" data-dismiss="modal">确认</button>
    ...
    <div class="pagination pagination-right">
        <ul>
       	 	<!-- 首页 -->
            <li>
            	<a href="<c:url value='/healthBureau/back_maintain.action?page=1' />">首页</a>
			</li>
			<!-- 上一页 --> 
            <li>
				<c:choose>
					<c:when test="${pageBean.currentPage == 1 }"><a href="javascipt:void(0)" class="previous disabled">上一页</a></c:when>
					<c:otherwise><a href="<c:url value='/healthBureau/back_maintain.action?page=${pageBean.currentPage - 1 }' />" >上一页</a></c:otherwise>
				</c:choose>
			</li>
			<!-- 中间页 -->
			<%--我们需要计算页码列表的开始和结束位置，即两个变量begin和end
			 ...
			 <c:forEach begin="${begin }" end="${end }" var="i">
			   <c:choose>
			   	  ...
			   	  <c:otherwise>
			   	    <li><a href="<c:url value='/healthBureau/back_maintain.action?page=${i }' /> ">${i }</a></li>
			   	  </c:otherwise>
			   </c:choose>
			 </c:forEach>
            ...
            <li>
				...
				<c:otherwise><a href="<c:url value='/healthBureau/back_maintain.action?page=${pageBean.currentPage + 1 }' />">下一页</a></c:otherwise>
			  </c:choose>
            	
            </li>
            <!-- 尾页 -->
            <li>
            	<a href="<c:url value='/healthBureau/back_maintain.action?page=${pageBean.totalPage }' /> ">尾页</a>
            </li>
        </ul>
       </div>
    </div>
...
<script type="text/javascript">
	...
	/* //更新订单的确认点击事件
	 function checkInputValue(){
		 var orderDto = {
					"orderDto.name" : $('input[name="orderDto.id"]').val(),
					"orderDto.type"	:	$('input[name="orderDto.hospitalId"]').val(),
					"orderDto.standard"	:	$('input[name="orderDto.order_Time"]').val(),
					"orderDto.stock_Balance"	:	$('input[name="orderDto.amount"]').val(),
					"orderDto.manufacturer.number"	:	$('input[name="orderDto.return_Time"]').val(),
					"orderDto.price"	  :		$('input[name="orderDto.return_Reason"]').val(),
					"orderDto.status"	:	$('input[name="orderDto.status"]')
		        };
			
			var message = "back";
			
			$.ajax({url:"<c:url value='/healthBureau/updateOrder.action' />",
				type:"POST",
				 data: orderDto,	
				async:true,
				cache:false,
				success:function(data){	
					 alert("成功！"); 
					location.replace("back_maintain.action"); 
				},
			});	 
	} */
	...
	function deleteOrder(id){
		...
		$.ajax({url:"<c:url value='/healthBureau/deleteOrderById.action' />",
			...
			success:function(data){
				alert("删除成功！"); 
				location.replace("back_maintain.action");  			//replace() 方法可用一个新文档取代当前文档。
			}
		});	

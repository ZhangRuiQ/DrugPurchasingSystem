...
<!DOCTYPE html>    
    ...
    <body>
     <form action="<c:url value='/healthBureau/purchase_put.action' />"  method="post" id="myForm" >
     ...
     <div class="block-content collapse in">
        <div>
           <a href="#" onclick="putOrders()" ><i class="icon-plus"></i>批量入库</a>
        </div>
        <br>
        <div >    
                ...
                <thead>
                  ...
                  <tbody id="tbody" >
	                <s:iterator value="pageBean.list" status="m" var="var" >
	                    <tr class="gradeA">
	                      	<td><input type="checkbox" id="cb${id }" /></td>
	                        ...
	                        <td></td>
	                        <td></td>
                        	<td><a href="#" id="${id }" onclick="click_put(this.id)" >入库</a> </td>
                    </tr>
                   </s:iterator>
                </tbody>
            </table>
    ...
    <div class="pagination pagination-right">
        <ul>
       	 	<!-- 首页 -->
            <li>
            	<a href="<c:url value='/healthBureau/purchase_put.action?page=1' />">首页</a>
			</li>
			<!-- 上一页 --> 
            <li>
				<c:choose>
					<c:when test="${pageBean.currentPage == 1 }"><a href="javascipt:void(0)" class="previous disabled">上一页</a></c:when>
					<c:otherwise><a href="<c:url value='/healthBureau/purchase_put.action?page=${pageBean.currentPage - 1 }' />" >上一页</a></c:otherwise>
				</c:choose>
			</li>
			<!-- 中间页 -->
			<%--我们需要计算页码列表的开始和结束位置，即两个变量begin和end
			 ...
			 <c:forEach begin="${begin }" end="${end }" var="i">
			   <c:choose>
			   	  ...
			   	  <c:otherwise>
			   	    <li><a href="<c:url value='/healthBureau/purchase_put.action?page=${i }' /> ">${i }</a></li>
			   	  </c:otherwise>
			   </c:choose>
			 </c:forEach>
            ...
            <li>
				...
				<c:otherwise><a href="<c:url value='/healthBureau/purchase_put.action?page=${pageBean.currentPage + 1 }' />">下一页</a></c:otherwise>
			  </c:choose>
            	
            </li>
            <!-- 尾页 -->
            <li>
            	<a href="<c:url value='/healthBureau/purchase_put.action?page=${pageBean.totalPage }' /> ">尾页</a>
            </li>
        </ul>
     </div>
    </div>
	...
	function click_put(id){
		 ...
		 $.ajax({url:"<c:url value='/healthBureau/updateOrder_status.action' />",
			...
			success:function(data){
				location.replace("purchase_put.action");  			//replace() 方法可用一个新文档取代当前文档。
			}
			});	 
	}
	...
	function putOrders(){
		...
		for(i=0; i < CheckBoxs.length; i++){
			...
			if( CheckBoxs[i].checked ){			//删掉选中的Medicine
				...
				click_put(CheckBoxs[i].id.substring(2));

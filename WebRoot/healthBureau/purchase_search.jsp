...
<!DOCTYPE html>    
    ...
    <body>
      <form action="<c:url value='/healthBureau/purchase_search.action' />" method="post" id="form">
    ...
    <div class="pagination pagination-right">
        <ul>
       	 	<!-- 首页 -->
            <li>
            	<a href="<c:url value='/healthBureau/purchase_search.action?page=1' />">首页</a>
			</li>
			<!-- 上一页 --> 
            <li>
				<c:choose>
					<c:when test="${pageBean.currentPage == 1 }"><a href="javascipt:void(0)" class="previous disabled">上一页</a></c:when>
					<c:otherwise><a href="<c:url value='/healthBureau/purchase_search.action?page=${pageBean.currentPage - 1 }' />" >上一页</a></c:otherwise>
				</c:choose>
			</li>
			<!-- 中间页 -->
			<%--我们需要计算页码列表的开始和结束位置，即两个变量begin和end
			 ...
			 <c:forEach begin="${begin }" end="${end }" var="i">
			   <c:choose>
			   	  ...
			   	  <c:otherwise>
			   	    <li><a href="<c:url value='/healthBureau/purchase_search.action?page=${i }' /> ">${i }</a></li>
			   	  </c:otherwise>
			   </c:choose>
			 </c:forEach>
            ...
            <li>
				...
				<c:otherwise><a href="<c:url value='/healthBureau/purchase_search.action?page=${pageBean.currentPage + 1 }' />">下一页</a></c:otherwise>
			  </c:choose>
            	
            </li>
            <!-- 尾页 -->
            <li>
            	<a href="<c:url value='/healthBureau/purchase_search.action?page=${pageBean.totalPage }' /> ">尾页</a>
            </li>
        </ul>
     </div>
    </div>   
...
<script type="text/javascript">
	...
	/* 错误提示：	window.onload使用了两次，而有一个使用了匿名函数，结果那匿名函数不起作用了。  */
	//全选、全不选和反选
	/* window.onload = function check_all(){		//页面加载后立即获取所需的变量值（如下变量）

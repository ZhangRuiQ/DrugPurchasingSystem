...
<!DOCTYPE html>    
    ...
    <body>
        <form action="<c:url value='/healthBureau/back_ratify.action' />"  method="post" id="myForm" >
                  ...
                  <tbody id="tbody" >
	                <s:iterator value="pageBean.list" status="m" var="var" >
                        ...
                        <ul class="dropdown-menu">
                            <li><a href="#" id="${id }pass" onclick="click_ratify(this.id)">通过</a></li>
                            <li><a href="#" id="${id }dispass" onclick="click_ratify(this.id)">不通过</a></li>
                        </ul>
                    </div>
                            </td>
                   		 </tr>
    ...
    <div class="pagination pagination-right">
        <ul>
       	 	<!-- 首页 -->
            <li>
            	<a href="<c:url value='/healthBureau/back_ratify.action?page=1' />">首页</a>
			</li>
			<!-- 上一页 --> 
            <li>
				<c:choose>
					<c:when test="${pageBean.currentPage == 1 }"><a href="javascipt:void(0)" class="previous disabled">上一页</a></c:when>
					<c:otherwise><a href="<c:url value='/healthBureau/back_ratify.action?page=${pageBean.currentPage - 1 }' />" >上一页</a></c:otherwise>
				</c:choose>
			</li>
			<!-- 中间页 -->
			<%--我们需要计算页码列表的开始和结束位置，即两个变量begin和end
			 ...
			 <c:forEach begin="${begin }" end="${end }" var="i">
			   <c:choose>
			   	  ...
			   	  <c:otherwise>
			   	    <li><a href="<c:url value='/healthBureau/back_ratify.action?page=${i }' /> ">${i }</a></li>
			   	  </c:otherwise>
			   </c:choose>
			 </c:forEach>
            ...
            <li>
				...
				<c:otherwise><a href="<c:url value='/healthBureau/back_ratify.action?page=${pageBean.currentPage + 1 }' />">下一页</a></c:otherwise>
			  </c:choose>
            	
            </li>
            <!-- 尾页 -->
            <li>
            	<a href="<c:url value='/healthBureau/back_ratify.action?page=${pageBean.totalPage }' /> ">尾页</a>
            </li>
        </ul>
    </div>
    </div>
	...
	function click_ratify(id){
		...
		if( "通过" == text ){
			...
			id = id.replace("pass","");
			 ...
			 $.ajax({url:"<c:url value='/healthBureau/updateOrder_status.action' />",
				type:"POST",
				data: {
					"orderDto.id" :  id ,
					"orderDto.status" : 70},		
				...
				success:function(data){
					alert("成功！"); 
					location.replace("back_ratify.action");  			//replace() 方法可用一个新文档取代当前文档。
				}
			});	 

		}else if( "不通过" == text ){
			
			id = id.replace("dispass","") ;
